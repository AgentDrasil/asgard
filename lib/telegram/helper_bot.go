...
func StartHelper(ctx context.Context, token string) error {
	...
	b, err := bot.New(token, bot.WithDefaultHandler(func(ctx context.Context, b *bot.Bot, update *models.Update) {
		...
		if update.Message.Text != "" {
			log.Info().
				Str("username", update.Message.From.Username).    // username
				Int64("id", update.Message.From.ID).              // user id
				Int64("chat_id", update.Message.Chat.ID).         // group id
				Int("thread_id", update.Message.MessageThreadID). // topic id
				Msg("Receive message from")
