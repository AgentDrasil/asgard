...
function syncArtifactFiles(autoOpenNew = false) {
  ...
  if (props.messages && props.messages.length > 0) {
    for (const msg of props.messages) {
      if (msg.targetFile) {
        ...
        if (!modifiedFiles.value.includes(msg.targetFile)) {
          hasNewFile = true;
          latestFile = msg.targetFile;
        }
      }
    }
  }
  ...
  // Auto-open drawer when a new artifact is generated by agent
  if (autoOpenNew && hasNewFile && latestFile) {
    activeArtifactPath.value = latestFile;
...
<template>
  ...
  <div v-else class="flex-1 flex flex-col h-full overflow-hidden relative">
    <!-- Top Area: Split ChatArea & ArtifactViewer -->
