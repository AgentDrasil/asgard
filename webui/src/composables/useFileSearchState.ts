...
export function useFileSearchState(sessionIdSource?: SessionIdSource, debounceMs = 200) {
  const query = ref("");
  ...
  const executeSearch = async (searchQuery: string) => {
    ...
    try {
      const data = await searchFiles(sid, trimmed, 50, controller.signal);
  ...
  const selectCurrent = (): FileSearchResult | null => {
